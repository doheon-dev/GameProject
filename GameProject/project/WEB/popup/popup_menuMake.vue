  ...
  methods: {
    confirm: function (done) {
      if (this.g_menu.menuid == '') {
        ...
        this.$confirm('메뉴를 추가하시겠습니까?', '메뉴 추가', {
          confirmButtonText: '확인', cancelButtonText: '취소', type: 'warning'
        }).then(() => {
          this.$http.post('/api/addMenu', this.g_menu).then(response => {
            if (response.status === 200) {
              this.$message({ type: 'success', message: '추가 되었습니다.' });
              this.$emit('setAddMenu');
              this.cancel();
            } else {
              this.$message({ type: 'warning', message: '추가가 실패하였습니다.' });
            }
          })
        }).catch(() => {
